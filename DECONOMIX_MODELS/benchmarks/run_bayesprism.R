...
option_list <- list(
  ...
  make_option("--out-dir", type = "character", default = "DECONOMIX_MODELS/results/benchmarks/bayesprism"),
...
message("[BayesPrism] Loading scRNA reference")
...
message("[BayesPrism] Using ", ncol(sce), " reference cells after capping")
...
message("[BayesPrism] Using assay=", assay_name)
...
bp <- new_BayesPrism(reference = ref_mat, input.type = "count.mat", cell.type.label = as.character(colData(sce)$cell_type))
bp <- run.bayesprism(bp, mixture = bulk_mat)
pred <- as.data.frame(t(bp@post.props$final_gibbs))
...
message("[BayesPrism] Saved results to ", out_dir)
