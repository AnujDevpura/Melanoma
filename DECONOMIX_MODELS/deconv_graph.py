...
for epoch in range(n_epochs):
    ...
    model.eval()
    with torch.no_grad():
        val_preds = model(X_val_t, adjacency_tensor).numpy()
        val_loss = criterion(model(X_val_t, adjacency_tensor), y_val_t).item()
...
model.eval()
with torch.no_grad():
    test_predictions = model(X_test_t, adjacency_tensor).numpy()
...
plt.tight_layout()
plt.savefig('gnn_deconvolution.png', dpi=300, bbox_inches='tight')
print("Saved: gnn_deconvolution.png")
...
plt.tight_layout()
plt.savefig('gene_coexpression_network.png', dpi=300, bbox_inches='tight')
print("Saved: gene_coexpression_network.png")
...
perf_df.to_csv('performance_gnn.csv', index=False)
